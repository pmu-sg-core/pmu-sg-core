...
- Use "pm.task_resume_pending" ONLY when task collection is already active (gatheringTask) AND the user's latest message cannot plausibly be an answer to the question you just asked in your previous turn AND has no clear connection to the task in progress. Never use this when the user's reply is a direct or plausible answer to your last question — even if it is a single word (e.g. "Critical" in response to a priority question is a direct answer, not ambiguous).
